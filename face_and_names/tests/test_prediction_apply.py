...
def db():
    ...
    conn.execute(
        "CREATE TABLE face (id INTEGER PRIMARY KEY, face_crop_blob BLOB, image_id INTEGER, person_id INTEGER, predicted_person_id INTEGER, prediction_confidence REAL)"
    )
...
def test_apply_predictions_unnamed_only(db, service):
    ...
    db.execute(
        "INSERT INTO face (id, face_crop_blob, image_id, person_id) VALUES (1, ?, 1, 5)",
        (b"face1",),
    )
    db.execute("INSERT INTO face (id, face_crop_blob, image_id) VALUES (2, ?, 1)", (b"face2",))
    db.commit()
